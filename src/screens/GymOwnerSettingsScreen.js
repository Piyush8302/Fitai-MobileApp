...
// Short date for the PDF tables, e.g. "5 Jan 26"
const fdate = (d) => d ? new Date(d).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: '2-digit' }) : '—';
// One table row for the member report (shared by single & combined PDFs)
const reportRow = (r, i) => {
  const st = r.status && r.status !== 'active'
    ? `<span style="color:#9092b0;font-weight:700">${String(r.status).toUpperCase()}</span>`
    : `<span style="color:${r.isDue ? '#E53935' : '#3E9D43'};font-weight:700">${r.isDue ? 'DUE' : 'PAID'}</span>`;
  return `
    <tr style="background:${i % 2 ? '#f5f6fb' : '#fff'}">
      ...
      <td style="text-align:right">₹${r.fee || 0}</td>
      <td>${fdate(r.joinDate)}</td>
      <td style="text-align:center">${r.present}</td>
      <td style="text-align:right">₹${r.paid}</td>
      <td>${fdate(r.lastPaidDate)}</td>
      <td style="color:${r.isDue ? '#E53935' : '#1b1d33'}">${fdate(r.dueDate)}</td>
      <td style="text-align:center">${st}</td>
    </tr>`;
};
const REPORT_HEAD = '<tr><th>#</th><th>Name</th><th>Phone</th><th>Plan</th><th>Fee</th><th>Joined</th><th>Present</th><th>Paid</th><th>Last Payment</th><th>Next Due</th><th>Status</th></tr>';
...
const GymOwnerSettingsScreen = ({ navigation }) => {
  ...
  const generateReport = async (gym, months = reportMonths) => {
    ...
    try {
      ...
      const rowsHtml = rows.map(reportRow).join('');
      ...
      const html = `
        ...
        <style>
          ...
          .card .v{font-size:20px;font-weight:700;color:#1554b8}
          .card .l{font-size:11px;color:#6b6b8d}
          table{width:100%;border-collapse:collapse;font-size:10.5px}
          th{background:#1554b8;color:#fff;padding:6px;text-align:left;white-space:nowrap}
          ...
          <div class="cards">
            ...
            <div class="card"><div class="v" style="color:#E53935">₹${totals.pendingAmount || 0}</div><div class="l">Pending Amount</div></div>
          </div>
          <table>
            ${REPORT_HEAD}
            ${rowsHtml || '<tr><td colspan="11" style="text-align:center;padding:20px">No members</td></tr>'}
          </table>
          <p style="margin-top:24px;color:#9092b0;font-size:11px">Generated by FitAI • ${new Date().toLocaleString('en-IN')}</p>
        </body></html>`;

      const { savedTo } = await downloadAndSharePdf(html, `${gym.name}-report-${months}mo`, `${gym.name} Report`);
      if (savedTo) Alert.alert('Downloaded', 'Report PDF saved to your device.');
    } catch (e) {
  ...
  const generateAllReport = async (months = reportMonths) => {
    ...
    try {
      ...
      const grandPending = results.reduce((a, r) => a + (r.totals.pendingAmount || 0), 0);
      const sections = results.map(r => {
        const rowsHtml = r.rows.map(reportRow).join('');
        return `
          <h2 style="color:#1554b8;margin:22px 0 2px">${r.gym.name}</h2>
          <div style="color:#6b6b8d;font-size:12px;margin-bottom:8px">${r.gym.location || ''} • ${r.totals.members} members • ₹${r.totals.totalCollected} collected • ${r.totals.totalDue} due • ₹${r.totals.pendingAmount || 0} pending</div>
          <table>
            ${REPORT_HEAD}
            ${rowsHtml || '<tr><td colspan="11" style="text-align:center;padding:12px">No members</td></tr>'}
          </table>`;
      }).join('');
      ...
      const html = `
        ...
        <style>
          ...
          <div class="cards">
            ...
            <div class="card"><div class="v" style="color:#E53935">₹${grandPending}</div><div class="l">Pending Amount</div></div>
          </div>
          ${sections}
          <p style="margin-top:24px;color:#9092b0;font-size:11px">Generated by FitAI • ${new Date().toLocaleString('en-IN')}</p>
        </body></html>`;

      const { savedTo } = await downloadAndSharePdf(html, `all-gyms-report-${months}mo`, 'All Gyms Report');
      if (savedTo) Alert.alert('Downloaded', 'Report PDF saved to your device.');
    } catch (e) {
      Alert.alert('Error', 'PDF generation needs the built APK. Rebuild to use reports.');
    } finally { setGenBusy(false); }
